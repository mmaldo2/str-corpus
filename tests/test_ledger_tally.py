...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=1498, machine_only=2764)   # 1373/2956 after the third budget; rounds 3 and 3b (2026-09-11)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1911   # 1925 after the third budget; rounds 3 and 3b (2026-09-11)
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 389    # 395 after the third budget; rounds 3 and 3b (2026-09-11)   # 138 before reference v2
    m = v.matrix()
