...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=1274, machine_only=2560)   # 1274/2099 after the shard-02 rounds; second tail budget (2026-09-09)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1666   # 1482 after the shard-02 rounds; second tail budget (2026-09-09)
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 363    # 328 after the shard-02 rounds; second tail budget (2026-09-09)   # 138 before reference v2
    m = v.matrix()
    pre = {k: t for k, t in m.cells.items() if k[0] == "pre-1860" and k[2] == "householder"}
    assert sum(t.human_reviewed + t.machine_only for t in pre.values()) == 20   # 17 after the re-read; the second tail budget admitted three more (2026-09-09)
