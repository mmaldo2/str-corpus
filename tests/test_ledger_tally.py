...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=133, machine_only=2806)   # 560 machine-only before the cycle-004 map admission
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1261   # 367 before the cycle-004 map admission
