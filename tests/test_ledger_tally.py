...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=821, machine_only=2598)   # 821/2645 after the shard-02 tail map; the cycles 1-3 re-read (2026-09-09)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1450   # 1501 after the shard-02 tail map; the cycles 1-3 re-read under mapper-v3 (2026-09-09)
