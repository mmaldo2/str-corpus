...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=555, machine_only=2308)   # 453/2410 after round 2; round 3a confirmed 102 (relevance overturns held for 3b)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1290   # 1267 after round 2; round 3a resolved 36 mixed
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 324    # 321 after round 2; round 3a   # 138 before reference v2
    m = v.matrix()
