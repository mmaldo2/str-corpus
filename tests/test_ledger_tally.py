...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=947, machine_only=2465)   # 821/2598 after the re-read admission; re-read round 1 agreed set (2026-09-09)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1458   # 1450 after the re-read admission; re-read round 1 agreed set (2026-09-09)
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 323    # 318 after the re-read admission; re-read round 1 agreed set (2026-09-09)   # 138 before reference v2
    m = v.matrix()
