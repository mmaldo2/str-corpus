...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=1373, machine_only=2956)   # 1373/2413 after rounds 2/2b; third tail budget (2026-09-11)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1925   # 1658 after rounds 2/2b; third tail budget (2026-09-11)
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 395    # 361 after rounds 2/2b; third tail budget (2026-09-11)   # 138 before reference v2
    m = v.matrix()
