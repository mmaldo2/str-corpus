...
def test_counts_and_matrix_on_the_real_ledger(repo_root):
    ...
    assert c.total == TierCount(human_reviewed=1373, machine_only=2413)   # 1274/2560 after the second budget; rounds 2 and 2b (2026-09-10)
    pol = v.counts(by=("polarity",))
    assert pol[("favorable",)].human_reviewed + pol[("favorable",)].machine_only == 1658   # 1666 after the second budget; rounds 2 and 2b (2026-09-10)
    hh = v.counts(polarity="favorable", who_was_letting="householder")
    assert hh.total.human_reviewed + hh.total.machine_only == 361    # 363 after the second budget; rounds 2 and 2b (2026-09-10)   # 138 before reference v2
    m = v.matrix()
