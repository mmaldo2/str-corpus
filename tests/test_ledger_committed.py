...
def test_the_rename_leaves_the_committed_counts_and_the_replay_untouched(repo_root):
    ...
    assert v.counts().total.human_reviewed + v.counts().total.machine_only == 3834   # 3373 after the shard-02 rounds; the second tail budget admitted 461 (2026-09-09)
    fav = v.counts(polarity="favorable").total
    assert fav.human_reviewed + fav.machine_only == 1666   # 1482 after the shard-02 rounds; second tail budget (2026-09-09)
    hh = v.counts(polarity="favorable", who_was_letting="householder").total
    assert hh.human_reviewed + hh.machine_only == 363    # 328 after the shard-02 rounds; second tail budget (2026-09-09)
...
def test_the_protection_rule_leaves_the_published_counts_untouched(repo_root):
    v = open_ledger(domain=load_domain()).view()
    assert v.counts().total.human_reviewed + v.counts().total.machine_only == 3834
    assert v.counts().total.human_reviewed == 1274
