...
def test_the_rename_leaves_the_committed_counts_and_the_replay_untouched(repo_root):
    ...
    assert v.counts().total.human_reviewed + v.counts().total.machine_only == 3373   # 3397 after the shard-02 agreed set; round 1b withdrew 20 more (2026-09-09)
    fav = v.counts(polarity="favorable").total
    assert fav.human_reviewed + fav.machine_only == 1482   # 1491 after the shard-02 agreed set; round 1b (2026-09-09)
    hh = v.counts(polarity="favorable", who_was_letting="householder").total
    assert hh.human_reviewed + hh.machine_only == 328    # 329 after the shard-02 agreed set; round 1b (2026-09-09)
...
def test_the_protection_rule_leaves_the_published_counts_untouched(repo_root):
    v = open_ledger(domain=load_domain()).view()
    assert v.counts().total.human_reviewed + v.counts().total.machine_only == 3373
