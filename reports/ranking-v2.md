...
**`ranking.classifier_version` moved to `v2` on 2026-09-08 by the user's decision ("Ship v2").** The controller's recommendation, adopted: D6 is met against fusion on both views; classifier v1's 0.8319 on this slice is not a clean number (32 of the 293 rows were labelled before v1 trained and were likely in its training set), while v2's 0.8274 excluded both frozen slices; the gap is inside the noise of a 293-row slice; and v2 was trained on about four times the positives, including every cycle-004 human label. The one point in v1's favour, precision at 50 (0.86 vs 0.82), matters little to a 3,000-case budget that spends far past the top 50. The cycle-004 tail (shard 02) is therefore re-ranked and read on classifier v2. That key is the operative pin: `--ranker classifier:v2` does not override it (`load_ranker` refuses a suffix that disagrees with it).
